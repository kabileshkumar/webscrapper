...
(async () => {
    ...
    const products = await page.evaluate(() => {
        ...
        productElements.forEach((el) => {
            ...
            const discountPrice = el.querySelector('div.Nx9bqj._4b5DiR')?.textContent?.trim(); // Discounted price
            const originalPrice = el.querySelector('div.yRaY8j.ZYYwLA')?.textContent?.trim(); // Original price

            if (name && discountPrice) {
                productList.push({
                    name: name,
                    discountPrice: discountPrice,
                    originalPrice: originalPrice || "Not available" // If original price is missing, set a default message
                });
            }
        });
    ...
    console.log(products);  // Print products for debugging

    // Save to file
    if (products.length === 0) {
        console.log("No products found. Check the page structure and CSS selectors.");
    } else {
